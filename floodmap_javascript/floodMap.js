var map, veTileLayer, parishLayersDetails, url, seclusionTaskUrl, loading, click = false;
var BingMaps;
var layer;
var info = [];
var d;
...
var date_urls = [];
var layerData = [];
require(["esri/map", "esri/virtualearth/VETiledLayer", "dojo/dom", "esri/tasks/query", "esri/tasks/QueryTask", "dojo/on", "dijit/form/HorizontalSlider", "esri/tasks/IdentifyTask", "esri/tasks/IdentifyParameters", "esri/dijit/InfoWindow", "esri/geometry/Point", "esri/symbols/PictureMarkerSymbol", "esri/symbols/TextSymbol", "esri/symbols/Font",
...
], function (Map, VETiledLayer, dom, Query, QueryTask, on, HorizontalSlider, IdentifyTask, IdentifyParameters, InfoWindow, Point, PictureMarkerSymbol, TextSymbol, Font, Color, InfoTemplate, Graphic, ArcGISDynamicMapServiceLayer, FeatureLayer, GraphicsLayer, webMercatorUtils, Search) {
    map = new Map("mapDiv", {
        center: [-90, 30.75],
        zoom: 9,
        basemap: "topo",
    ...
    var search = new Search({
        ...
        value: "Find Address or (Lat, Long)"
    }, "txtAddress");
    search.startup();
    search.on('select-result', addPoint);
    search.on('search-results', findPoint);
        ...
        if (!click) {
            ...
            function compl(e) {
                ...
                for (var i = 0 ; i < layerData.length ; i++) {
                    ...
                    catch (ex) { console.log("Map click is outside the area") };
                }
                json.cont = cont;
                getServices();
            }
            function getServices() {
                ...
                $.when(s_wind, s_cid, s_elev).then(function () {
                    ...
                    var attr = { "ID": length, "Information": json.cont };
                    var sms = new PictureMarkerSymbol('images/pin8.png', 48, 48);
                    sms.setOffset(0, 20);                    
                    var point = esri.geometry.webMercatorToGeographic(mapClick);
                    var title = "Pt. " + map.graphics.graphics.length + " (" + point.y.toFixed(4) + "," + point.x.toFixed(4) + ")";
                    var infotemp = new InfoTemplate(title, getInfoContent(mapClick, json));
                    var graphic = new Graphic(evt, sms, attr, infotemp);                   
                    map.graphics.add(graphic);
                    //Add graphic count on Pop up
                    var textSymbolFont = new Font("14pt", esri.symbol.Font.STYLE_NORMAL, esri.symbol.Font.VARIANT_NORMAL, esri.symbol.Font.WEIGHT_BOLD, "Verdana, Geneva, Arial, Helvetica, sans-serif");
                    var textSymbol = new TextSymbol((parseInt(map.graphics.graphics.length / 2)).toString(), textSymbolFont);
                    textSymbol.setOffset(0, 20),
                    textSymbol.setColor(new dojo.Color([236, 240, 241]));
                    var textgraphic = new Graphic(evt, textSymbol);
    ...
    function addPoint(e) {
        try {
            var parishIdentify = new Query();
            var parishTask = new QueryTask("http://terra2.agcenter.lsu.edu/arcgis/rest/services/DEV/BaseServices/MapServer/0");
            parishIdentify.geometry = e.result.feature.geometry;
            parishIdentify.where = '1=1';
            parishIdentify.spatialRelationship = Query.SPATIAL_REL_INTERSECTS;
            parishIdentify.outFields = ["*"];
            parishIdentify.returnGeometry = true;
            parishTask.execute(parishIdentify, getLayer);
            function getLayer(res) {
                if (res.features.length > 0) {
                    if (res.features[0].attributes.DFIRM_URL != "") {
                        url = res.features[0].attributes.DFIRM_URL;
                        addLayer(url);
                        excuteIdentify(e.result.feature.geometry);
                    }
                }
                else {
                    excuteIdentify(e.result.feature.geometry);
    ...
    function addPointLayer(e) {
        try {
            excuteIdentify(e.mapPoint);
        }
        catch (ex) {
            alert(ex.message);
        }
    ...
    function addPointsTemp() {
        if (map.graphics.graphics.length > 0) {
            $('#mapPoints').empty();
            var data = map.graphics.graphics;
            for (var i = 0; i < data.length; i++) {
                $('#mapPoints').append('<div class="pointstable-wrap"><table><tr class="header"><td><h3><span>' + data[i].infoTemplate.title + '</span></h3></td></tr><tr class="child" style="display:none"><td>' + data[i].infoTemplate.content + '</td></tr></div>');
            }
        }
        else {
            $('#mapPoints').empty();
            $('#mapPoints').html("<h4>Click Map to add Point</h4>");
        }
    }
...
$(document).ready(function () {
    $("#mapPoints").on("click", ".header", function () {
        if ($(this).hasClass("headerOpen")) {
